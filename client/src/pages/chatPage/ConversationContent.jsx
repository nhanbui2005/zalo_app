...
const ConversationContent = ({ avatarUrl, name, type, partnerId, roomId }) => {
  ...
  useSocketEvent(
    `event:${roomId}:writing_message`,(data) => {      
  ...
  const MessageItem = ({ data }) => {
    const { content, type, status, sender, isSelfSent, createdAt, isLastest } = data
    return (
      <div className={`flex flex-col`}>
        ...
        <div className={`flex ${isSelfSent && 'flex-row-reverse'}`}>
          {
            (isSelfSent && isLastest) && <p className={`text-white bg-dark-5 p-1 rounded-md text-xs`}>Đã gửi</p>
          }
        </div>
      </div>
    )
    ...
    <div className="mx-0.5 flex w-full flex-col">
      ...
      <div className="h-full overflow-auto p-8 scrollbar-hide">
        {[...messages].reverse().map((item, index) => (
          <MessageItem key={index.toString()} data={{...item,isLastest: index == messages.length - 1}} />
        ))}
      ...
      </div>
      {
        isPartnerWrite &&
        <div className='flex'>
          <p className='text-white px-2 bg-dark-5'>Đang soạn tin nhắn</p>
        </div>
      }
      {/* nhập tin nhắn */}
      <div
