import { IEmailJob, ISendOTPEmailJob, ISendPasswordEmailJob, IVerifyEmailJob } from '@/common/interfaces/job.interface';
...
export class EmailProcessor extends WorkerHost {
  ...
  async process(
    ...
    switch (job.name) {
      ...
      case JobName.EMAIL_SEND_PASSWORD:
        return await this.emailQueueService.sendEmailSendPassword(
          job.data as unknown as ISendPasswordEmailJob,
        );  
      case JobName.EMAIL_SEND_OTP_FORGOT_PASSWORD:
        return await this.emailQueueService.sendOTPForgotPassword(
          job.data as unknown as ISendOTPEmailJob,
        );  
      default:
